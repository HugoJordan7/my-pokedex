...
class DetailsPresenter(override var view: DetailsFragment) : DetailsContract.Presenter {
    ...
    override fun findWeaknesses(pokemon: Pokemon) {
        ...
        val secondType: String? =
            if (pokemon.types.size > 1) pokemon.types[1].name.name.toLowerCase() else null
    ...
    override fun onSuccessWeaknesses(primary: Type, second: Type) {
        val primaryListWeak =
            primary?.relations?.doubleDamageFrom?.map { it?.name } as MutableList<String>
        val secondListStrong =
            second?.relations?.halfDamageFrom?.map { it?.name } as MutableList<String>
        primaryListWeak.removeAll(secondListStrong)

        val secondListWeak =
            second?.relations?.doubleDamageFrom?.map { it?.name } as MutableList<String>
        val primaryListStrong =
            primary?.relations?.halfDamageFrom?.map { it?.name } as MutableList<String>
        ...
        if (weaknessesList.isNotEmpty()) {
            ...
            val noDamagePrimaryList =
                primary?.relations?.noDamageFrom?.map { it?.name } ?: emptyList()
            val noDamageSecondList =
                second?.relations?.noDamageFrom?.map { it?.name } ?: emptyList()
    ...
    fun onSuccessPokemonEvolution(response: List<Pokemon>) {
        if (response.size > 1) {
            val listEvolutionItem = response.map {
                EvolutionItem(
                    it.name, it.id, ProjectResources.getPokeImgUrlById(it.id)
                )
            }
            val columns = if(response.size == 2) 2 else 3
    ...
    override fun onFailure(message: String) {
        if (view.isVisible) view.showFailure(message)
    }
